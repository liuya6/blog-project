...
import { renderDefaultColumns } from '../components/renderDefaultColumns';
...
export function useColumns(
  ...
  const getViewColumns = computed(() => {
    ...
    return columns
      ...
      .map((column) => {
        const { slots, customRender, format, edit, editRow, flag, dataIndex, showTips } = column;
        ...
        if (showTips && !(edit || editRow) && !customRender && !format && dataIndex) {
          column.customRender = renderDefaultColumns(column);
        }

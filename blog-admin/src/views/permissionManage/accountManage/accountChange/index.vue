<script lang="tsx">
  ...
  export default defineComponent({
    setup() {
      ...
      return () => (
        <PageWrapper>
          <CollapseContainer title={isEdit.value ? '编辑账号' : '新增账号'}>
      ...
      async function handleSubmit(params: any) {
        try {
          let fn = permissionService.accountAdd;
          if (isEdit.value) {
            fn = permissionService.accountEdit;
            params.id = systemManageStore.editAccount.id;
          }
          const data = await fn(params);
          if (data.code === ResultEnum.SUCCESS) {
            createMessage.success(isEdit.value ? '修改成功' : '新增成功');
            go('/permissionManage/accountManage');
          }
        } catch (e) {
          console.warn(e);
        }
